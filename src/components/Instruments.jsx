 ...
 * эти величины только текстом. Инструменты ниже — не декор: спарклайн
 * показывает тренд, шкала — заполнение, полоса занятости — конфликты по
 * дням, тепловая матрица — сезонность объекта.
 *
 * Все инструменты работают на светлой поверхности и на корпусе (onDark).
 */

/** Тон инструмента: пара цветов для светлой поверхности и для корпуса */
const TONES = {
...
function toneColor(tone, onDark) {
  const pair = TONES[tone] ?? TONES.beam
  return onDark ? pair.dark : pair.light
}
...
export function Sparkline({
  ...
  const color = toneColor(tone, onDark)
        ...
        <>
          <defs>
            <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={hexToRgba(color, onDark ? 0.35 : 0.22)} />
    ...
    <div className={cn('min-w-0', className)}>
      {label || valueLabel ? (
        <div className="mb-1.5 flex items-baseline justify-between gap-2">
          <span
            className={cn(
              'truncate text-2xs font-semibold uppercase tracking-label',
              onDark ? 'text-obsidian-300' : 'text-ink-400',
            )}
          >
            {label}
          </span>
      ...
      <div
        className={cn(
          ...
          onDark ? 'bg-white/[0.08]' : 'bg-ink-200/70',
        )}
      >
        <div
          ...
          style={{
            width: `${percent}%`,
            backgroundImage: `linear-gradient(90deg, ${hexToRgba(color, 0.75)} 0%, ${color} 100%)`,
            boxShadow: `0 0 8px 0 ${hexToRgba(color, onDark ? 0.55 : 0.35)}`,
          }}
        />
        {threshold != null ? (
          <span
            ...
            className={cn(
              ...
              onDark ? 'bg-obsidian-50/60' : 'bg-ink-900/45',
            )}
      ...
      {hint ? (
        <p className={cn('mt-1 truncate text-2xs', onDark ? 'text-obsidian-300' : 'text-ink-400')}>
          {hint}
        </p>
      ) : null}
    </div>
          ...
          return (
            <span
              ...
              className={cn(
                ...
                !meta && (onDark ? 'bg-white/[0.06]' : 'bg-ink-200/60'),
  ...
  return (
    <div className={cn('inline-flex flex-col items-center', className)}>
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} className="-rotate-90" aria-hidden="true">
          <circle
            ...
            fill="none"
            strokeWidth={stroke}
            stroke={onDark ? 'rgba(255,255,255,0.08)' : ink[200]}
          />
          <circle
            cx={size / 2}
            cy={size / 2}
            ...
            style={{
              ...
              '--ring-offset': `${offset}`,
              filter: `drop-shadow(0 0 5px ${hexToRgba(color, onDark ? 0.6 : 0.4)})`,
            }}
          />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {sublabel ? (
            <span
              className={cn(
                'mt-0.5 text-2xs uppercase tracking-label',
                onDark ? 'text-obsidian-300' : 'text-ink-400',
              )}
            >
              {sublabel}
      ...
      {label ? (
        <span
          className={cn(
            'mt-2 text-center text-2xs font-semibold uppercase tracking-label',
            onDark ? 'text-obsidian-200' : 'text-ink-500',
          )}
        >
          {label}
        </span>
            ...
            <tr>
              ...
              {columns.map((column) => (
                <th
                  key={column}
                  className={cn(
                    'pb-0.5 text-center text-2xs font-semibold uppercase tracking-normal',
                    onDark ? 'text-obsidian-300' : 'text-ink-400',
                  )}
                >
                  {column}
          ...
          {rows.map((row) => (
            <tr key={row.label}>
              <th
                scope="row"
                className={cn(
                  'pr-2 text-right font-mono text-2xs font-semibold whitespace-nowrap',
                  onDark ? 'text-obsidian-100' : 'text-ink-700',
                )}
              >
                {row.label}
              </th>
              {row.values.map((intensity, index) => {
                const alpha = onDark ? 0.06 + intensity * 0.82 : 0.05 + intensity * 0.8
                return (
                  <td key={index} className="p-0">
                    <span
                      ...
                      style={{
                        ...
                        boxShadow:
                          intensity > 0.75
                            ? `0 0 8px 0 ${hexToRgba(color, onDark ? 0.5 : 0.3)}`
                            : undefined,
                      }}
                    />
                  </td>
        ...
        {data.map((entry, index) => {
          ...
          return (
            <span
              ...
              className={cn(
                'min-w-0 flex-1 origin-bottom rounded-t-xs animate-grow-y transition-[filter] duration-fast hover:brightness-125',
              )}
              style={{
                height: `${Math.max(4, (value / max) * 100)}%`,
                animationDelay: `${index * 28}ms`,
                backgroundColor: isActive ? color : hexToRgba(color, onDark ? 0.4 : 0.28),
            ...
            <span
              ...
              className={cn(
                ...
                highlightIndex === index
                  ? onDark
                    ? 'text-obsidian-50 font-semibold'
